...
voodoo_generate(uint8_t *code_block, voodoo_t *voodoo, voodoo_params_t *params, voodoo_state_t *state, int depthop)
{
    ...
    if ((params->fbzMode & FBZ_W_BUFFER) || (params->fogMode & (FOG_ENABLE | FOG_CONSTANT | FOG_Z | FOG_ALPHA)) == FOG_ENABLE) {
        ...
        addbyte(0);
        // Here laid "addbyte(4+5+2+3+2+5+5+3+2+2+2+/*3+*/3+2+6+4+5+2+3);", perhaps one of the worst and most inexplicable lines of code I have ever laid my eyes on
        ...
        addbyte(0);
        // also here laid addbyte(5+5+3+2+2+2+/*3+*/3+2+6+4+5+2+3);
